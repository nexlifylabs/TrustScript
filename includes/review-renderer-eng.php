...
class TrustScript_Review_Renderer {
	...
	public static function _on_comment_meta_change( $meta_id, $comment_id, $meta_key, $meta_value ) {
		...
		if ( $should_flush ) {
			$rating_keys = array( 'rating', '_trustscript_rating', '_trustscript_verified_purchase', '_trustscript_verification_hash' );
			if ( in_array( $meta_key, $rating_keys, true ) ) {
				self::flush_store_stats_cache();
			}
	...
	/**
	 * Computes and caches store-wide aggregate review stats.
	 *
	 * @since 1.0.0
	 * @return array
	 */
	private static function get_store_stats() {
		$cached = get_transient( 'trustscript_store_stats' );
		if ( false !== $cached ) {
			return $cached;
		}

		if ( get_transient( 'trustscript_store_stats_lock' ) ) {
			return array(
				'rating'          => 0,
				'total'           => 0,
				'verified_pct'    => 0,
		...
		set_transient( 'trustscript_store_stats_lock', 1, 30 );

		global $wpdb;
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- This query is complex and optimized for performance, and is not expected to be run frequently due to caching.
		$stats = $wpdb->get_row(
			$wpdb->prepare(
				"SELECT
					COUNT(rating_data.comment_ID) AS total_reviews,
					AVG(rating_data.rating) AS avg_rating,
					SUM(CASE WHEN rating_data.rating >= 4 THEN 1 ELSE 0 END) AS recommend_count
				FROM (
					SELECT c.comment_ID, MAX(CAST(m.meta_value AS DECIMAL(3,1))) AS rating
					FROM {$wpdb->comments} c
					INNER JOIN {$wpdb->commentmeta} m
						ON c.comment_ID = m.comment_id
						AND m.meta_key IN ('rating', '_trustscript_rating')
				...
				WHERE rating_data.rating BETWEEN 1 AND 5",
				'product',
				'publish',
				'1'
			)
		...
		$verified_count = (int) $wpdb->get_var(
			$wpdb->prepare(
				"SELECT COUNT(DISTINCT m.comment_id)
				...
				'product',
				'publish',
				'_trustscript_verified_purchase',
				'_trustscript_verification_hash',
				'1'
			)
		);
		...
		$data  = array(
			'rating'        => $total > 0 ? round( (float) $stats->avg_rating, 1 ) : 0,
			'total'         => $total,
			'verified_pct'  => $total > 0 ? round( ( $verified_count / $total ) * 100 ) : 0,
			'recommend_pct' => $total > 0 ? round( ( (int) $stats->recommend_count / $total ) * 100 ) : 0,
		);

		set_transient( 'trustscript_store_stats', $data, 6 * HOUR_IN_SECONDS );
		delete_transient( 'trustscript_store_stats_lock' );

		return $data;
	}

	/**
	 * Flushes the store-wide stats cache.
	 *
	 * @since 1.0.0
	 */
	public static function flush_store_stats_cache() {
		delete_transient( 'trustscript_store_stats' );
		delete_transient( 'trustscript_store_stats_lock' );
	}

	/**
	 * Renders the store-wide trust strip.
	 *
	 * @since 1.0.0
	 * @return string HTML
	 */
	private static function render_trust_strip() {
		if ( ! get_option( 'trustscript_enable_trust_strip', true ) ) {
        return '';
    }
		...
		ob_start();
		?>
		<div class="trustscript-trust-strip">
			<div class="trustscript-trust-strip-stat">
				<div class="trustscript-trust-strip-value"><?php echo esc_html( number_format( $stats['rating'], 1 ) ); ?></div>
				<div class="trustscript-trust-strip-stars">
					<?php echo wp_kses_post( self::render_stars( (int) round( $stats['rating'] ) ) ); ?>
				</div>
				<div class="trustscript-trust-strip-label"><?php esc_html_e( 'Store rating', 'trustscript' ); ?></div>
			</div>
			<div class="trustscript-trust-strip-stat">
				<div class="trustscript-trust-strip-value"><?php echo esc_html( number_format_i18n( $stats['total'] ) ); ?></div>
				<div class="trustscript-trust-strip-label"><?php esc_html_e( 'Total reviews', 'trustscript' ); ?></div>
			</div>
			<div class="trustscript-trust-strip-stat">
				<div class="trustscript-trust-strip-value"><?php echo esc_html( $stats['verified_pct'] ); ?>%</div>
				<div class="trustscript-trust-strip-label"><?php esc_html_e( 'Verified buyers', 'trustscript' ); ?></div>
			</div>
			<div class="trustscript-trust-strip-stat">
				<div class="trustscript-trust-strip-value"><?php echo esc_html( $stats['recommend_pct'] ); ?>%</div>
				<div class="trustscript-trust-strip-label"><?php esc_html_e( 'Recommend this', 'trustscript' ); ?></div>
			</div>
		</div>
		<?php
		return ob_get_clean();
	...
	public static function render_schema_markup() {
		...
		// Output the JSON-LD schema markup in the page header. 
		// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- wp_json_encode() provides sufficient escaping for JSON-LD context.
	...
	public static function render_review_section( $product_id, $options = array() ) {
		...
		if ( is_user_logged_in() ) {
			...
			if ( ! empty( $user_reviews ) ) {
				...
				foreach ( $reviews as $k => $rev ) {
					if ( (int) $rev->comment_ID === (int) $user_review->comment_ID ) {
						unset( $reviews[ $k ] );
					}
				}
		...
		if ( $total > 0 && false === $cached_stats ) {
			$all_reviews_for_stats = get_comments(
				array(
					'post_id' => $product_id,
					'status'  => 'approve',
					'type'    => 'review',
		...
		<div class="trustscript-reviews-wrap" id="reviews">
			...
			<div class="trustscript-section-header-row">
				<div> 
					<?php if ( ! empty( $options['heading'] ) ) : ?>
						<h2 class="trustscript-section-title"><?php echo esc_html( $options['heading'] ); ?></h2>
					<?php endif; ?>
					<?php if ( ! empty( $options['subheading'] ) ) : ?>
						<p class="trustscript-section-sub"><?php echo esc_html( $options['subheading'] ); ?></p>
					<?php endif; ?>
				</div>
				<?php if ( class_exists( 'TrustScript_Simple_Review' ) ) : ?>
					<?php echo TrustScript_Simple_Review::render_write_review_button( $product_id ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
				<?php endif; ?>
			</div>

			<?php if ( $total > 0 ) : ?>
				<?php if ( ! empty( $all_media ) ) : ?>
					<?php echo self::render_trust_strip(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
					<div class="trustscript-top-grid">
				...
				<?php else : ?>
					<div class="trustscript-top-grid trustscript-no-media trustscript-trust-strip-only">
						<?php echo self::render_trust_strip(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
					</div>
				<?php endif; ?>
			<?php endif; ?>

			<?php if ( ! empty( $keywords ) && $total > 0 ) : ?>
			...
			<div class="trustscript-reviews-list" id="trustscript-reviews-list"
				...
				if ( ! empty( $user_review ) ) {
					echo '<div class="trustscript-user-review-card" style="margin-bottom:24px;">';
					echo '<div class="trustscript-user-review-notice" style="margin-bottom:12px;padding:12px 16px;background:#eff6ff;border:1px solid #bfdbfe;border-radius:8px;font-size:14px;color:#1e3a8a;display:flex;justify-content:space-between;align-items:center;">';
					echo '<span style="font-weight:500;">' . esc_html__( 'You have already reviewed this product.', 'trustscript' ) . '</span>';

					$rating = (int) get_comment_meta( $user_review->comment_ID, '_trustscript_rating', true );
					if ( ! $rating ) {
						$rating = (int) get_comment_meta( $user_review->comment_ID, 'rating', true );
					}
					echo '<button type="button" class="trustscript-edit-review-btn" style="background:#2563eb;color:#fff;border:none;padding:6px 12px;border-radius:4px;font-size:13px;font-weight:500;cursor:pointer;" data-comment-id="' . esc_attr( $user_review->comment_ID ) . '" data-rating="' . esc_attr( $rating ) . '" data-text="' . esc_attr( $user_review->comment_content ) . '">' . esc_html__( 'Edit Review', 'trustscript' ) . '</button>';
					echo '</div>';

					echo '<div class="trustscript-review-card-wrapper">';
					echo self::render_card( $user_review, $options ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					echo '</div>';
					echo '</div>';
				}

				if ( empty( $reviews ) && empty( $user_review ) ) {
					echo '<p class="trustscript-no-reviews">' . esc_html__( 'No reviews yet. Be the first!', 'trustscript' ) . '</p>';
				} else {
					$i = 0;
	...
	public static function render_card( $review, $options = array() ) {
		...
		<div class="trustscript-review-card"
			...
			data-trustscript="<?php echo esc_attr( $ts ); ?>"
					...
					<div class="trustscript-reviewer-info">
						...
						<?php if ( $is_verified ) : ?>
							...
							<button type="button"
									...
									title="<?php esc_attr_e( 'Verify this review', 'trustscript' ); ?>">
			...
			<div class="trustscript-photo-carousel"
				...
				<div class="trustscript-carousel-viewport"
					...
					<?php
					foreach ( $all_media as $i => $media_url ) :
					...
					<div class="trustscript-carousel-thumb<?php echo $i === 0 ? ' trustscript-thumb-active' : ''; ?>"
						...
						<div style="position:absolute;top:0;left:0;width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,0.3);border-radius:6px;">
							<?php echo trustscript_get_play_svg(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
						</div>
						<?php else : ?>
				...
				<div class="trustscript-helpful-row">
					...
					<?php echo trustscript_get_thumbs_up_svg(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
				...
				<button type="button"
					...
					<?php echo trustscript_get_thumbs_down_svg(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
	...
	private static function render_summary_card( $avg, $total, $counts ) {
		...
		<div class="trustscript-summary-card">
			...
			$sentiments = array(
				array(
					'label' => __( 'Positive', 'trustscript' ),
					'pct'   => $pos_pct,
					'mod'   => 'positive',
				),
				array(
					'label' => __( 'Neutral', 'trustscript' ),
					'pct'   => $neu_pct,
					'mod'   => 'neutral',
				),
				array(
	...
	private static function render_gallery_card( $display_media, $display_meta = array(), $total_media_count = 0, $product_id = 0 ) {
		...
		<div class="trustscript-gallery-card"
			...
			<div class="trustscript-gallery-header">
				...
				<span class="trustscript-gallery-count">
					...
					printf(
						...
						esc_html__( '%d images', 'trustscript' ),
						absint( $total_count )
					);
					?>
				</span>
			...
			<div class="trustscript-gallery-grid">
				<?php
				foreach ( array_slice( $display_media, 0, $display_count ) as $i => $url ) :
			...
			<div class="trustscript-modal">
				...
				<div class="trustscript-modal-footer">
					<a href="<?php echo esc_url( $verify_url ); ?>" id="trustscript-verify-link-btn" class="trustscript-verify-external-link" target="_blank" rel="noopener noreferrer">
						<?php esc_html_e( 'Verify this review ↗', 'trustscript' ); ?>
					</a>
				</div>
	...
	private static function render_verify_modal_shell() {
		// Only render the modal if verification modal is enabled in settings.
