...
class TrustScript_Pending_Queue_Page {
	...
	public static function render() {
		...
		<div class="wrap trustscript-queue-wrap">
			...
			<table class="widefat striped wp-list-table trustscript-queue-table" id="trustscript-queue-table">
				<thead>
					<tr>
						<th scope="col"><?php esc_html_e( 'ID', 'trustscript' ); ?></th>
						<th scope="col"><?php esc_html_e( 'Order #', 'trustscript' ); ?></th>
						<th scope="col"><?php esc_html_e( 'Service', 'trustscript' ); ?></th>
						<th scope="col"><?php esc_html_e( 'Reason', 'trustscript' ); ?></th>
						<th scope="col"><?php esc_html_e( 'Queued At', 'trustscript' ); ?></th>
						<th scope="col"><?php esc_html_e( 'Scheduled For', 'trustscript' ); ?></th>
						<th scope="col"><?php esc_html_e( 'Last Attempt', 'trustscript' ); ?></th>
						<th scope="col"><?php esc_html_e( 'Retries', 'trustscript' ); ?></th>
						<th scope="col"><?php esc_html_e( 'Status', 'trustscript' ); ?></th>
							...
							<tr id="trustscript-queue-row-<?php echo esc_attr( $item['id'] ); ?>"
								...
								<td>
									...
									$reason_labels    = array(
										...
										'simple'     => __( 'Simple Email', 'trustscript' ),
									);
									$reason_class_map = array(
										...
										'simple'     => 'trustscript-queue-reason-simple-email',
									);
								...
								<td>
									<?php
									$sf_raw = $item['scheduled_for'];
									if ( empty( $sf_raw ) || '0000-00-00 00:00:00' === $sf_raw ) {
										echo '<em class="trustscript-queue-placeholder-text">&mdash;</em>';
									} else {
										$sf_formatted = TrustScript_Date_Formatter::format( $sf_raw, 'datetime' );
	...
	public function handle_queue_retry() {
		$nonce_verified = wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ?? '' ) ), 'trustscript_admin' );
		...
		if ( ! isset( $providers[ $service_id ] ) ) {
			if ( 'simple' === $service_id ) {
				$success = class_exists( 'TrustScript_Simple_Email_Review' )
					? TrustScript_Simple_Email_Review::send_review_request( $order_id )
					: false;
				if ( $success ) {
					...
					if ( $order ) {
						...
						$order->update_meta_data( '_trustscript_review_sent_at', wp_date( 'Y-m-d H:i:s' ) );
					...
					wp_send_json_success(
						array(
							'removed' => true,
							'message' => sprintf( /* translators: %d: order ID */ __( 'Order #%d email sent successfully.', 'trustscript' ), $order_id ),
						)
					);
				} else {
					wp_send_json_error( array( 'message' => __( 'Failed to send simple review email.', 'trustscript' ) ) );
				}
			}
	...
	public function handle_queue_process_all() {
		...
		// Use the same shared lock as the cron to prevent double-processing if a cron
		// run is already in progress. The lock TTL is 15 minutes in the cron, so a
		// busy response here is safe and transient.
		$lock_key = 'trustscript_queue_lock';
		if ( get_transient( $lock_key ) ) {
			wp_send_json_error( array( 'message' => __( 'Queue is currently being processed. Please wait a moment and try again.', 'trustscript' ) ), 503 );
		}
		set_transient( $lock_key, 1, 15 * MINUTE_IN_SECONDS );

		try {
			$results = TrustScript_Queue::process_batch( 20, false );
			...
			wp_send_json_error( array( 'message' => __( 'An error occurred while processing the queue. Try again later.', 'trustscript' ) ), 500 );
		}
	}
}