...
class TrustScript_Block_Checkout_Consent {
	...
	public static function capture_block_consent( $order, $request ) {
		...
		$trustscript_data = isset( $extensions[ self::STORE_API_NAMESPACE ] )
			? $extensions[ self::STORE_API_NAMESPACE ]
			: array();

		if ( empty( $trustscript_data ) ) {
			self::record_no_consent_required( $order );
			return;
		}

		$order_id        = $order->get_id();
		$nonce           = isset( $trustscript_data['nonce'] ) ? $trustscript_data['nonce'] : '';
		$consent_given   = ! empty( $trustscript_data['consent_given'] );
		$billing_country = sanitize_text_field(
			! empty( $trustscript_data['consent_country'] ) ? $trustscript_data['consent_country'] : $order->get_billing_country()
		);
		$consent_type    = sanitize_text_field(
			! empty( $trustscript_data['consent_type'] ) ? $trustscript_data['consent_type'] : 'not_required'
		);
