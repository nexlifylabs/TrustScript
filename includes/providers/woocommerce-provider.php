...
class TrustScript_WooCommerce_Provider extends TrustScript_Service_Provider {
	...
	public function handle_item_refund( $order_id, $refund_id ) {
		...
		foreach ( $refund->get_items() as $refund_item ) {
			...
			$product_token = $original_item->get_meta( '_trustscript_product_token' );
			if ( empty( $product_token ) ) {
				continue;
			}

			$qty_original    = (int) $original_item->get_quantity();
			...
			if ( $code >= 200 && $code < 300 ) {
				$original_item->delete_meta_data( '_trustscript_product_token' );
				$original_item->save();
			}
		}
	}
	 ...
	 * Each product array includes: productId, parentProductId, productName, productSku, productImageUrl, quantity
	 * Note: parentProductId is included to help identify the main product for variations, since WooCommerce reviews are always stored against the parent product.
	 * 
	 * @param int|WC_Order $order_id Order ID or order object
	...
	public function extract_all_products( $order_id ) {
		...
		foreach ( $order->get_items() as $item ) {
			...
			$parent_product_id = $variation_id > 0 ? (int) $item->get_product_id() : (int) $product->get_id();
	...
	/**
	 * Get all eligible product IDs from an order based on category filtering, free product exclusion,
	 * refund status, and minimum value. This is used for the legacy single-product review request API
	 * to determine which product ID to send in the payload.
	 *
	 * @param int|WC_Order $order_id Order ID or order object
