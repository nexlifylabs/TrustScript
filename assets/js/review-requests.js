...
(function ($) {
	...
	var isLoading = false;
	var STATUS_CFG = {
		pending: { label: 'Pending', bg: '#eff6ff', fg: '#3b82f6' },
		scheduled: { label: 'Scheduled', bg: '#fffbeb', fg: '#f59e0b' },
		sent: { label: 'Sent', bg: '#ecfdf5', fg: '#10b981' },
		published: { label: 'Published', bg: '#d1fae5', fg: '#065f46' },
		'opt-out': { label: 'Opt-Out', bg: '#faf5ff', fg: '#8b5cf6' },
		already_published: { label: 'Already Reviewed', bg: '#f3f4f6', fg: '#6b7280' },
		ineligible: { label: 'Ineligible', bg: '#f3f4f6', fg: '#9ca3af' },
		cancelled: { label: 'Cancelled', bg: '#fef2f2', fg: '#dc2626' },
	};

	var CONSENT_CFG = {
		'N/A': { label: 'N/A', bg: '#f3f4f6', fg: '#6b7280', tooltip: 'Consent not required for this country' },
		'Not Given': { label: 'Not Given', bg: '#fef2f2', fg: '#dc2626', tooltip: 'Customer did not check the consent checkbox' },
		'Confirmed': { label: 'Confirmed', bg: '#d1fae5', fg: '#065f46', tooltip: 'Customer confirmed consent (double opt-in link clicked)' },
		'Waiting': { label: 'Waiting', bg: '#fef3c7', fg: '#b45309', tooltip: 'Awaiting customer confirmation of double opt-in' },
		'Expired': { label: 'Expired', bg: '#fecaca', fg: '#991b1b', tooltip: 'Double opt-in confirmation link expired (7 days)' },
		'Pending': { label: 'Pending', bg: '#eff6ff', fg: '#3b82f6', tooltip: 'Consent pending' },
	};
		...
		$.post(
			...
			function (response) {
				...
				$('#rr-results-info').html(
					'Showing <strong>' + start + '–' + end + '</strong> of <strong>' + data.total + '</strong> entries'
				);

				renderTable(data.orders, data.canSendSimple);
	...
	function renderTable(orders, canSendSimple) {
		var html = '';
		...
		orders.forEach(function (order) {
			...
			var INELIGIBLE_REASON_LABELS = {
				category_filter: 'Excluded by category filter',
				free_product: 'Free product — excluded by settings',
				min_value: 'Item value below minimum threshold',
				fully_refunded: 'Fully refunded',
			};

			// Tooltip for informational-only statuses.
			var badgeTooltip = '';
			if (isAlreadyPublished) { badgeTooltip = ' title="Customer has already reviewed this product" style="cursor:help;"'; }
			if (isOptOut) { badgeTooltip = ' title="Customer opted out of review requests"'; }
			if (isCancelled) { badgeTooltip = ' title="This item was refunded or the order was cancelled"'; }
			...
			if (isAlreadyPublished) {
				...
				actionHtml = '<button type="button" class="button button-small" disabled title="Customer already reviewed this product" style="cursor:not-allowed;opacity:0.5;">Already Reviewed</button>';
			} else if (isOptOut || isCancelled || isIneligible) {
				actionHtml = '<span style="color:#ddd;">—</span>';
			} else if (isPublished && order.productUrl) {
				actionHtml = '<a href="' + window.tsEscapeHtml(order.productUrl) + '" target="_blank" class="trustscript-action-link" title="View review on product page">View</a>';
				...
				actionHtml =
					...
					'data-order-id="' + window.tsEscapeHtml(String(rawOrderId)) + '">Send Email</button>';
			} else if (isScheduled && !order.emailSent) {
				// Email scheduled for sending - show disabled button with tooltip
				actionHtml = '<button type="button" class="button button-small" disabled title="Email scheduled for sending" style="cursor:not-allowed;opacity:0.5;">Send Email</button>';
			} else if (canSendSimple && isPending && !order.emailSent && order.displayConsentStatus === 'Waiting') {
				// Consent pending - show disabled button with tooltip
				actionHtml = '<button type="button" class="button button-small" disabled title="Awaiting customer confirmation of double opt-in" style="cursor:not-allowed;opacity:0.5;">Send Email</button>';
			} else if (canSendSimple && isPending && !order.emailSent && order.displayConsentStatus === 'Expired') {
				// Confirmation link expired
				actionHtml = '<button type="button" class="button button-small" disabled title="Double opt-in confirmation link expired" style="cursor:not-allowed;opacity:0.5;">Send Email</button>';
			} else if (canSendSimple && isPending && !order.emailSent && order.displayConsentStatus === 'Declined') {
				// Customer declined
				actionHtml = '<button type="button" class="button button-small" disabled title="Customer declined consent" style="cursor:not-allowed;opacity:0.5;">Send Email</button>';
			} else {
				actionHtml = '<span style="color:#ddd;">—</span>';
			}
	...
	function renderPagination(currentPg, totalPages) {
		...
		if (currentPg > 1) { html += btn(currentPg - 1, '← Previous', false); }
		...
		if (currentPg < totalPages) { html += btn(currentPg + 1, 'Next →', false); }
	...
	$(document).ready(function () {
		...
		$(document).on('click', '.trustscript-send-simple-email-btn', function () {
			...
			$.post(TrustscriptAdmin.ajax_url, {
				...
				if (res.success) {
					$btn.text('Sent!').addClass('button-primary');
